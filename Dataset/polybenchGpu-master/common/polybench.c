...
int gettimeofday(struct timeval* tp, void* tzp)
{
    ...
    tmp |= ((unsigned __int64)ft.dwHighDateTime << 32);
    tmp |= ft.dwLowDateTime;

    // convert to microseconds
    tmp /= 10;

    // convert epoch from 1601 → 1970
    tmp -= 11644473600000000ULL;

    tp->tv_sec = (long)(tmp / 1000000UL);
    tp->tv_usec = (long)(tmp % 1000000UL);
...
#ifdef _WIN32
  newA = _aligned_malloc(num, 32);
  if (!newA)
    {
      fprintf(stderr, "[PolyBench] aligned_malloc: cannot allocate memory");
      exit(1);
    }
#else
