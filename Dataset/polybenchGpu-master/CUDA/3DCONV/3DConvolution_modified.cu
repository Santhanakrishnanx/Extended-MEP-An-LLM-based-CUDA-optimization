...
__global__ void convolution3D_kernel(int ni, int nj, int nk, float* A, float* B, int i)
{
    const float c11 = +2.f, c12 = -3.f, c13 = +4.f;
    const float c21 = +5.f, c22 = +6.f, c23 = +7.f;
    const float c31 = -8.f, c32 = -9.f, c33 = +10.f;
    ...
    int slice_stride = nk * nj;
    int idx_center = i * slice_stride + j * nk + k;

    // Load neighbors from layer i-1
    float A_i_1_j_1_k_1 = A[(i - 1) * slice_stride + (j - 1) * nk + (k - 1)];
    float A_i_1_j_1_k_p1 = A[(i - 1) * slice_stride + (j - 1) * nk + (k + 1)];
    float A_i_1_j_0_k_p1 = A[(i - 1) * slice_stride + (j + 0) * nk + (k + 1)];
    float A_i_1_j_p1_k_p1 = A[(i - 1) * slice_stride + (j + 1) * nk + (k + 1)];

    // Load neighbors from layer i+1
    float A_i_p1_j_1_k_1 = A[(i + 1) * slice_stride + (j - 1) * nk + (k - 1)];
    float A_i_p1_j_1_k_p1 = A[(i + 1) * slice_stride + (j - 1) * nk + (k + 1)];
    float A_i_p1_j_0_k_p1 = A[(i + 1) * slice_stride + (j + 0) * nk + (k + 1)];
    float A_i_p1_j_p1_k_p1 = A[(i + 1) * slice_stride + (j + 1) * nk + (k + 1)];

    // Load neighbors from layer i+0
    float A_i_0_j_m1_k_0 = A[i * slice_stride + (j - 1) * nk + k];
    float A_i_0_j_0_k_0 = A[i * slice_stride + j * nk + k];
    float A_i_0_j_1_k_0 = A[i * slice_stride + (j + 1) * nk + k];
