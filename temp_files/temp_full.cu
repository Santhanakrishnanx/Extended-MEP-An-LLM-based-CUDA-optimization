...
__global__ void mm3_kernel3(int ni, int nj, int nl, int nm, int nk, float *E, float *F, float *G)
{
    // Each thread computes a 2x2 block of G to increase register reuse and reduce global memory loads
    // BlockDim.x and BlockDim.y must be multiples of 32 for warp efficiency; assume 16x16 or 32x32 blocks

    int i_base = blockIdx.y * blockDim.y * 2 + threadIdx.y * 2; // row base for 2 rows
    int j_base = blockIdx.x * blockDim.x * 2 + threadIdx.x * 2; // col base for 2 cols

    // Registers to hold partial sums for 4 output elements
    float g00 = 0.0f, g01 = 0.0f, g10 = 0.0f, g11 = 0.0f;

    // Loop over k dimension (nj), accumulate partial sums
    for (int k = 0; k < nj; ++k)
    {
        // Load E[i_base + 0][k] and E[i_base + 1][k] if in bounds
        ...
        // Load F[k][j_base + 0] and F[k][j_base + 1] if in bounds
        ...
        // Accumulate partial sums for 4 elements of G
        if (i_base < ni && j_base < nl)
            g00 += e0 * f0;
        if (i_base < ni && j_base + 1 < nl)
            g01 += e0 * f1;
        if (i_base + 1 < ni && j_base < nl)
            g10 += e1 * f0;
        if (i_base + 1 < ni && j_base + 1 < nl)
            g11 += e1 * f1;
    }

    // Write results back to G if in bounds
    if (i_base < ni && j_base < nl)
        G[i_base * nl + j_base] = g00;
    if (i_base < ni && j_base + 1 < nl)
        G[i_base * nl + j_base + 1] = g01;
    if (i_base + 1 < ni && j_base < nl)
        G[(i_base + 1) * nl + j_base] = g10;
    if (i_base + 1 < ni && j_base + 1 < nl)
        G[(i_base + 1) * nl + j_base + 1] = g11;
}
